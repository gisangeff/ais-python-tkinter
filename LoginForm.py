...
def LoginForm():
    ...
    Label(window, text="Username: ", bg="#880e0e", fg="#ffffff").place(x=15, y=67)
    Label(window, text="Password: ", bg="#880e0e", fg="#ffffff").place(x=15, y=90)

    txtUsername = Entry(window, relief="flat")
    txtUsername.place(x=85, y=65)
    txtPassword = Entry(window, relief="flat", show='*')
    txtPassword.place(x=85, y=90)   

    Button(window, text="Cancel", width=8, relief="flat", command=quit).place(x=38, y=125)
    Button(window, text="Login", width=8, relief="flat",command=loginVerify).place(x=128, y=125)
